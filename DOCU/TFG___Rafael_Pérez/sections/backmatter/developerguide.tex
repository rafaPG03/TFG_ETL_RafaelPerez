...
A continuación se recogen las instrucciones necesarias para evolucionar el \textit{software}. Este manual está dirigido a los desarrolladores que pretenden extender o modificar el código fuente, con el fin de incorporar nuevas funcionalidades o modificar las ya existentes.

\section{Introducción}
El presente proyecto consiste en una aplicación móvil multiplataforma orientada al análisis avanzado de estadísticas de fútbol, exactamente de LaLiga, mediante \textit{data mining} y algoritmos de \textit{machine learning}. La arquitectura sigue un modelo cliente-servidor y se divide en cuatro componentes principales:
\begin{itemize}
    \item Una aplicación móvil desarrollada con React Native y Expo, encargada de la presentación de la información y de la interacción con el usuario.
    \item Una API REST desarrollada con Node.js y Express, responsable de procesar las peticiones, aplicar la lógica de negocio y acceder a los datos.
    \item Una base de datos PostgreSQL organizada mediante tablas dimensionales, históricas y analíticas, en la que se almacena la información deportiva, los usuarios, los favoritos y los resultados de los procesos de \textit{data mining}.
    \item Un conjunto de servicios de análisis e inteligencia artificial compuesto por un simulador Monte Carlo implementado en Python y un asistente conversacional basado en Gemini 2.5 Flash.
\end{itemize}

\section{Preparaci\'on del entorno de trabajo}

La infraestructura del proyecto se ha contenerizado mediante Docker, evitando que
el desarrollador tenga que instalar localmente PostgreSQL, Node.js, Python o sus
dependencias. La arquitectura esta formada por tres contenedores comunicados
...
f\'isico. Esta se conecta al servidor Metro para descargar la aplicaci\'on
...
\begin{table}[H]
\centering
\caption{Requisitos de \textit{software} del entorno de desarrollo.}
...
Node.js, Python, PostgreSQL y las dependencias del proyecto no tendr\'an que
instalarse directamente en el equipo anfitri\'on, ya que estar\'an incluidos en
las im\'agenes Docker. Las versiones de los entornos utilizados dentro de los
contenedores se recogen en la Tabla~\ref{tab:entornos-internos}.
...
el script de simulaci\'on. La estructura es la mostrada en la Figura~\ref{fig:estructura-repositorio}.
...
b\'usqueda, chatbot y \textit{data mining}. Las rutas de la API permanecer\'an agrupadas
...
Las principales tecnolog\'ias utilizadas por el sistema se recogen en la tabla ~\ref{tab:stack-tecnologico}.
\begin{table}[H]
\centering
\caption{Stack tecnológico del proyecto.}
\label{tab:stack-tecnologico}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|>{\centering\arraybackslash}m{3.2cm}|p{4.2cm}|p{6cm}|}
\hline
\textbf{Capa} & \textbf{Tecnología} & \textbf{Propósito} \\
\hline

\multirow[c]{2}{3.2cm}{\centering Contenerización} 
& Docker Engine & Aislamiento y ejecución de los servicios. \\ \cline{2-3}
& Docker Compose & Configuración conjunta del frontend, backend y base de datos. \\
\hline

\multirow[c]{4}{3.2cm}{\centering Cliente} 
& React 19.1 & Construcción declarativa de la interfaz. \\ \cline{2-3}
& React Native 0.81.5 & Desarrollo de la aplicación móvil multiplataforma. \\ \cline{2-3}
& Expo SDK 54 & Entorno de desarrollo y ejecución mediante Expo Go. \\ \cline{2-3}
& Metro Bundler & Empaquetado y distribución del código JavaScript. \\
\hline

\multirow[c]{3}{3.2cm}{\centering Servidor} 
& Node.js & Entorno de ejecución del backend. \\ \cline{2-3}
& Express 5.2.1 & Implementación de la API REST. \\ \cline{2-3}
& dotenv y CORS & Gestión de variables de entorno y comunicación entre cliente y servidor. \\ \cline{2-3}
& Swagger y OpenAPI 3.0 & Documentación y prueba interactiva de los endpoints. \\ \cline{2-3}
& PostgreSQL 18.1 & Almacenamiento de datos deportivos, usuarios y resultados analíticos. \\
\hline

\multirow[c]{2}{3.2cm}{\centering Analítica} 
& Python 3.10, NumPy y pandas & Procesamiento de datos, generación de métricas y simulaciones. \\ \cline{2-3}
& psycopg2 & Comunicación entre los procesos Python y PostgreSQL. \\
\hline
